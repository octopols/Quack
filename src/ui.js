...
class CommentSearchUI {
  ...
  createSearchBox() {
    ...
    searchContainer.innerHTML = `
      ...
      <button id="quack-settings-button" class="quack-icon-button" title="Settings">
        <svg viewBox="0 0 24 24" width="20" height="20">
  ...
  createSettingsPopup(settings = {}) {
    ...
    const useRegexChecked = settings.useRegex ? 'checked' : '';
    const wholeWordChecked = settings.wholeWord ? 'checked' : '';
    ...
    popup.innerHTML = `
      ...
      <div class="quack-settings-body">
        ...
        <div class="quack-collapsible-content collapsed" id="quack-export-content">
          <div id="quack-download-loading" class="quack-download-loading" style="display: none;">
            <div class="quack-download-spinner"></div>
            <span id="quack-download-progress">Fetching...</span>
          </div>
          <div id="quack-download-options" class="quack-download-grid">
            <button id="quack-fetch-all" class="quack-dl-btn" title="Download all comments as JSON">
              <span>All JSON</span>
            </button>
            <button id="quack-fetch-all-csv" class="quack-dl-btn" title="Download all comments as CSV">
              <span>All CSV</span>
            </button>
            <button id="quack-download-results" class="quack-dl-btn" disabled title="Download search results as JSON">
              <span>Results JSON</span>
              <span class="quack-dl-count" id="quack-results-count"></span>
            </button>
            <button id="quack-download-results-csv" class="quack-dl-btn" disabled title="Download search results as CSV">
    ...
    // Store references to elements inside popup
    this.regexToggle = popup.querySelector('#quack-use-regex');
    this.wholeWordToggle = popup.querySelector('#quack-whole-word');
    this.fetchAllBtn = popup.querySelector('#quack-fetch-all');
  ...
  /**
   * Show download loading state in settings popup
   */
  showDownloadLoading() {
